...
const StorePage = () => {
  ...
  const getTagline = (gameName) => {
    const taglines = {
      'Grand Theft Auto V': 'La obra maestra del crimen y la libertad',
      'default': 'Más de 100 horas de juego abierto'
    }
    return taglines[gameName] || taglines['default']
  }
        ...
        <div className="relative z-20 h-full max-w-container-max mx-auto px-gutter flex items-end pb-16 md:pb-20">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-end w-full">
            ...
            <div className="space-y-4 md:space-y-6">
              ...
              <div className="flex items-center gap-3 animate-slide-in-up delay-100">
                {featuredGames[currentSlide].isBestseller && (
                  <span className="inline-flex items-center gap-1 bg-amber-500/20 backdrop-blur-sm text-amber-300 border border-amber-500/30 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider">
                    <span className="material-symbols-outlined text-sm">local_fire_department</span>
                    Bestseller
                  </span>
                )}
                {featuredGames[currentSlide].isExclusive && (
                  <span className="inline-flex items-center gap-1 bg-purple-500/20 backdrop-blur-sm text-purple-300 border border-purple-500/30 px-3 py-1.5 rounded-full text-xs font-bold uppercase tracking-wider">
              ...
              {/* Tagline emocional */}
              <p className="font-body-lg text-primary/90 max-w-xl leading-relaxed text-lg md:text-xl font-medium animate-slide-in-up delay-400">
                {featuredGames[currentSlide].tagline}
              </p>

              {/* Descripción (categoría) */}
              <p className="font-body-md text-on-surface-variant/70 max-w-xl leading-relaxed text-sm md:text-base animate-slide-in-up delay-500">
                {featuredGames[currentSlide].description}
              </p>

              {/* Rating con estrellas */}
              <div className="flex items-center gap-3 animate-slide-in-up delay-600">
                <div className="flex items-center gap-1">
                  {[...Array(5)].map((_, i) => (
                    <span
                      key={i}
                      className={`material-symbols-outlined text-lg ${
                        i < Math.floor(featuredGames[currentSlide].rating)
                          ? 'text-amber-400'
                          : 'text-on-surface-variant/30'
                      }`}
                    >
                      {i < Math.floor(featuredGames[currentSlide].rating) ? 'star' : 'star_border'}
                    </span>
                  ))}
                </div>
                <span className="text-on-surface-variant/80 text-sm font-medium">
                  {featuredGames[currentSlide].rating}/5
                </span>
                <span className="text-on-surface-variant/50 text-sm">
                  ({featuredGames[currentSlide].reviewCount.toLocaleString()} reseñas)
                </span>
              </div>

              {/* Precios y botones */}
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-4 pt-4 animate-slide-in-up delay-700">
                {/* Precio actual */}
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {config.waNumber && (
                    <button
                      ...
                      className="group inline-flex items-center gap-3 bg-[#25D366] hover:bg-[#20bd5a] text-white px-6 py-3.5 rounded-xl font-bold transition-all duration-300 hover:scale-105 animate-pulse-glow-green"
                    >
                      <svg 
                        className="w-6 h-6 group-hover:scale-110 group-hover:rotate-6 transition-transform" 
                        viewBox="0 0 24 24" 
                        fill="currentColor"
                      >
                      ...
                      Ver precio y ofertas
                    </button>
                  )}
                  
                  {/* Botón secundario - Ver tráiler */}
                  <button
                    type="button"
                    className="group inline-flex items-center gap-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white border border-white/20 px-5 py-3.5 rounded-xl font-medium transition-all duration-300 hover:scale-105"
                  >
          ...
          <div className="glass-panel p-6 rounded-xl sticky top-8 space-y-8">
            ...
            <div>
              ...
              <div className="space-y-1">
                {['all', 'PC', 'PS4','PS3','PS5','Licencias'].map((platform) => (
                  <label 
                    key={platform} 
                    className={`
